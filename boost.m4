...
# BOOST_WSERIALIZATION([PREFERRED-RT-OPT])
# ---------------------------------------
# Look for Boost.WSerialization.  For the documentation of PREFERRED-RT-OPT, see
# the documentation of BOOST_FIND_LIB above.
BOOST_DEFUN([WSerialization],
[AC_REQUIRE([BOOST_SERIALIZATION])dnl
wserialization_LIBS=$LIBS
wserialization_LDFLAGS=$LDFLAGS
m4_pattern_allow([^BOOST_(SYSTEM|SERIALIZATION)_(LIBS|LDFLAGS)$])dnl
LIBS="$LIBS $BOOST_SYSTEM_LIBS $BOOST_SERIALIZATION_LIBS"
LDFLAGS="$LDFLAGS $BOOST_SYSTEM_LDFLAGS $BOOST_SERIALIZATION_LDFLAGS"
BOOST_FIND_LIB([wserialization], [$1],
                [boost/archive/text_woarchive.hpp],
                [std::wostream* o = 0; // Cheap way to get an ostream...
                boost::archive::text_woarchive t(*o);])
LIBS=$wserialization_LIBS
LDFLAGS=$wserialization_LDFLAGS

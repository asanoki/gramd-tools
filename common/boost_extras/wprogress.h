//  boost wprogress.hpp header file  ------------------------------------------//
...
#ifndef BOOST_WPROGRESS_HPP
#define BOOST_WPROGRESS_HPP
...
private:
	std::wostream & m_os; // may not be present in all imps
	const std::wstring m_s1; // string is more general, safer than
	const std::wstring m_s2; //  const char *, and efficiency or size are
	const std::wstring m_s3; //  not issues
	...
	void display_tic() {
		...
		unsigned int tics_needed =
				static_cast<unsigned int>((static_cast<double>(_count)
						/ _expected_count) * 50.0 );do {m_os << '*' << std::flush;}while ( ++_tic < tics_needed );
				_next_tic_count =
